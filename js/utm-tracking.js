(function(){
  ...
  function setupCtaClickTracking(){ document.addEventListener('click', (e)=>{ const link=e.target && e.target.closest ? e.target.closest('a') : null; if(!link) return; const source=link.getAttribute('data-source')||''; if(link.classList.contains('cta-ext')){ (window.dataLayer=window.dataLayer||[]).push({event:'ext_cta_click', source}); } if(link.classList.contains('cta-record')){ (window.dataLayer=window.dataLayer||[]).push({event:'record_cta_click', source}); } }); }
  function init(){ const current=extractUTMFromURL(); if(Object.keys(current).length){ saveUTMParams(current); }
    setupExternalLinkTracking(); setupCtaClickTracking(); const saved=getSavedUTMParams(); if(Object.keys(saved).length){ (window.dataLayer=window.dataLayer||[]).push({event:'page_view_with_utm', page_location:window.location.href, page_title:document.title, utm_params:saved}); } }
